 ...
 */
function normalizePickPayload(event) {
    const payload = asRecord(event);
    const platform = payload.platform;
    const playerId = String(payload.player_id ?? payload.playerId ?? payload.playerID ?? payload.id ?? "").trim();
    const playerName = String(payload.player_name ?? payload.playerName ?? payload.name ?? "").trim();
    const position = normalizePosition(payload.position ?? payload.pos ?? payload.slot);
    const teamIndex = toInt(payload.team_index ?? payload.teamIndex ?? payload.draft_slot ?? payload.draftSlot);
    const round = toInt(payload.round ?? payload.rnd ?? payload.round_number) || 1;
    const pickNumber = toInt(payload.pick_number ?? payload.pickNumber ?? payload.overall_pick ?? payload.overallPick);
    const rawTs = payload.timestamp ?? payload.picked_at ?? payload.ts;
    const timestamp = typeof rawTs === "number" && Number.isFinite(rawTs) ? rawTs : undefined;
    ...
    const normalized = {
        ...
        player_id: playerId,
        position,
        fantasy_points: typeof payload.fantasy_points === "number" ? payload.fantasy_points : undefined,
        timestamp,
    };
    if (platform)
        normalized.platform = platform;
    if (playerName)
        normalized.player_name = playerName;
    if (!remember(fingerprint(normalized))) {
        return null;
    }
    return normalized;
}
